...
- 重构移动端阅读控制，新增音量键/滚轮翻页与九宫格点击区域自定义
- 新增阅读布局预设与多套颜色主题，支持保存/应用/删除/重置
- 新增长按文本上下文菜单，支持复制、全文搜索与段落评论
- 重构移动端分页/滚动阅读器，抽取分页测量、脚注布局与阅读位置等模块
- 补充 Cloudflare API Worker，支持段落计数与随机背景图接口
