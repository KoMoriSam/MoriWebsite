...
- 继续解耦阅读器组件，统一跨端排版与 Markdown 渲染
- 将提示块、聊天、朋友圈与代码块迁移为可挂载的 Vue Markdown 组件
- 优化阅读位置追踪，支持按视口保存与精确恢复，模式切换时保持阅读位置
- 统一模态框关闭与浏览器返回历史处理，避免关闭弹窗时的路由和滚动跳转
- 改进移动端目录手势、阅读进度条、章节控制与文章翻页交互
