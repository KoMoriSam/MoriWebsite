...
当窗口宽度缩小时，还需要观察图片和正文是否会发生过度挤压。例如在移动端，可以选择取消浮动，让图片恢复为普通块级内容；也可以保留较小尺寸的环绕布局。这里继续补充一些文字，以确保段落高度能够覆盖整张图片，从而更容易发现文字覆盖图片、浮动溢出或者后续元素意外进入环绕区域等问题。

这一段位于两组环绕内容之后，用来确认浮动已经被正确清除。它应该恢复正常的整行排版，不再受到前面任何一张图片的影响。

### `SVG` 格式的自适应反色

#### 原生引用

![殖民视角下的斯里兰卡历史阶段（前 3000 年至今）](/mock/article/images/sri-lankan-history-stages.svg)

#### WikiLink 引用

![[/mock/article/images/sri-lankan-history-stages.svg|殖民视角下的斯里兰卡历史阶段（前 3000 年至今）|center]]
